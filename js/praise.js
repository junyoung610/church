// junyoung610/.../js/praise.js - 최종 완성 및 오류 수정 버전
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const currentPath = window.location.pathname; // ----------------------------------------------------- // II. 글쓰기/수정 페이지 (praise/write.html) 로직 // -----------------------------------------------------

  if (currentPath.includes("praise/write.html")) {
  ...
  } // ----------------------------------------------------- // III. 목록 페이지 (praise/list.html) 로직 // -----------------------------------------------------

  if (currentPath.includes("praise/list.html")) {
    ...
    const praiseRef = db.collection("praise").orderBy("createdAt", "desc");
    ...
    async function loadPage(pageNumber) {
      let query = praiseRef.limit(POSTS_PER_PAGE);

      if (pageNumber > 1 && pageSnapshots[pageNumber - 2]) {
        query = praiseRef.startAfter(pageSnapshots[pageNumber - 2]).limit(POSTS_PER_PAGE);
      }
      ...
      try {
        ...
        snapshot.forEach((doc, index) => {
          ...
          html += `<tr><td class="col-num">${postNumber}</td><td class="col-title"><a href="./praise/view.html?id=${docId}">${post.title}</a></td><td class="col-author">${authorDisplay}</td><td class="col-date">${createdDate}</td></tr>`;
        });
  ...
  } // ----------------------------------------------------- // IV. 상세 보기 페이지 (praise/view.html) 로직 // -----------------------------------------------------

  if (currentPath.includes("praise/view.html")) {
    const urlParams = new URLSearchParams(window.location.search);
    const postId = urlParams.get("id");
